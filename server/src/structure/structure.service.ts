...
export class StructureService {
	...
	async createStructure(
		...
		const newStructure = await this.structureModel.create(structure);
		if (scheduleAdd) {
			const schedule = (
				await this.scheduleModel.find().sort({ date: -1 }).limit(1)
			)[0];
			const weeks = [...schedule.weeks];
			for (let i = 0; i < weeks.length; i++) {
				weeks[i].push({
					shift: newStructure._id.toString(),
					days: ['', '', '', '', '', '', ''],
				});
			}
	...
	async updateManyStructures(structures: Structure[]): Promise<Structure[]> {
		const structures_temp: Structure[] = [];
		for (let i = 0; i < structures.length; i++) {
			structures_temp.push(
				(await this.structureModel.findByIdAndUpdate(
					structures[i]._id,
					structures[i],
					{ new: true },
				))!,
	...
	async updateStructure(structure: Structure): Promise<Structure | null> {
		const structureFound = this.structureModel.findById(structure.id);
		if (!structureFound) {
			throw new NotFoundException('משמרת לא נמצאה');
		}
		return this.structureModel.findByIdAndUpdate(structure.id, structure, {
			new: true,
		});
	}
