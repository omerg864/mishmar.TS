...
import { AdminManagerMiddleware } from '../middleware/auth.middleware';
...
export class ShiftModule implements NestModule {
	configure(consumer: MiddlewareConsumer) {
		consumer
			...
			.forRoutes({ path: 'api/shifts/all', method: RequestMethod.GET });
		consumer
			.apply(SiteManagerMiddleware)
			.forRoutes({
		...
		consumer
			.apply(SiteManagerMiddleware)
			.forRoutes({ path: 'api/shifts/excel', method: RequestMethod.PUT });
		consumer
			.apply(AuthMiddleware)
			.forRoutes({ path: 'api/shifts', method: RequestMethod.PATCH });
		consumer
			.apply(AuthMiddleware)
			.forRoutes({
