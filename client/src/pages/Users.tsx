...
const Users = (props: IProps) => {
  ...
  const getUsers = async (loading: boolean) => {
    ...
    try {
      ...
      if (data.error || data.statusCode) {
        fetch(`${process.env.REACT_APP_API_URL}/api/logs`, { headers: { 'Content-Type': 'application/json' },method: 'POST', body: JSON.stringify({user: cookies.get('user'), err: data, path: `users/all`, component: "Users" })})
        toast.error(data.message);
      } else {
        setUsers(data);
      }
    } catch (err) {
      fetch(`${process.env.REACT_APP_API_URL}/api/logs`, { headers: { 'Content-Type': 'application/json' },method: 'POST', body: JSON.stringify({user: cookies.get('user'), err, path: `users/all`, component: "Users" })})
  ...
  const saveUsers = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.error || data.statusCode) {
        fetch(`${process.env.REACT_APP_API_URL}/api/logs`, { headers: { 'Content-Type': 'application/json' },method: 'POST', body: JSON.stringify({user: cookies.get('user'), err: data, path: `users/many`, component: "Users" })})
        toast.error(data.message);
      } else {
        toast.success("עודכן");
      ...
      fetch(`${process.env.REACT_APP_API_URL}/api/logs`, { headers: { 'Content-Type': 'application/json' },method: 'POST', body: JSON.stringify({user: cookies.get('user'), err, path: `users/many`, component: "Users" })})
  ...
  const changePassword = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/users/manager`, { headers: { "Content-Type": "application/json", authorization: 'Bearer ' + cookies.get('userToken') }, 
      method: 'PATCH', body: JSON.stringify(modal.user)});
      const data = await response.json();
      if (data.error || data.statusCode) {
        fetch(`${process.env.REACT_APP_API_URL}/api/logs`, { headers: { 'Content-Type': 'application/json' },method: 'POST', body: JSON.stringify({user: cookies.get('user'), err: data, path: `users/manager`, component: "Users" })})
        toast.error(data.message);
      } else {
      ...
      fetch(`${process.env.REACT_APP_API_URL}/api/logs`, { headers: { 'Content-Type': 'application/json' },method: 'POST', body: JSON.stringify({user: cookies.get('user'), err, path: `users/manager`, component: "Users" })})
      toast.error('Internal Server Error');
    }
    setIsLoading(false);
  }
  ...
  const deleteUser = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/users/${(e.target as HTMLButtonElement).value}`, { headers: { "Content-Type": "application/json", authorization: 'Bearer ' + cookies.get('userToken') }, 
      method: 'DELETE'});
      const data = await response.json();
      if (data.error || data.statusCode) {
        fetch(`${process.env.REACT_APP_API_URL}/api/logs`, { headers: { 'Content-Type': 'application/json' },method: 'POST', body: JSON.stringify({user: cookies.get('user'), err: data, path: `users/${(e.target as HTMLButtonElement).value}`, component: "Users" })})
        toast.error(data.message);
      } else {
      ...
      fetch(`${process.env.REACT_APP_API_URL}/api/logs`, { headers: { 'Content-Type': 'application/json' },method: 'POST', body: JSON.stringify({user: cookies.get('user'), err, path: `users/${(e.target as HTMLButtonElement).value}`, component: "Users" })})
      toast.error('Internal server error');
    }
    setIsLoading(false);
  }
